...
# How to Install 
First set up your device to be able to use private golang packages from `gitlab.snapp.ir`. for more information, check this [wiki](https://gitlab.snapp.ir/Map/sdk/smapp-sdk-go/-/wikis/Private-Golang-Modules)
...
require (
    ...
	gitlab.snapp.ir/Map/sdk/smapp-sdk-go v0.3.0
    ...
)

replace gitlab.snapp.ir/Map/sdk/smapp-sdk-go => gitlab.snapp.ir/Map/sdk/smapp-sdk-go.git v0.3.0
```
...
+ `SMAPP_API_KEY_SOURCE`: specifies the source of api key in a request. valid values are `header` and `query`. default value is `header`.
+ `SMAPP_API_KEY_NAME`: specifies the name of api key in a request. default value is `X-Smapp-Key`.
+ `SMAPP_API_REGION`: the region of smapp api should be specified here. valid values are `teh-1` and `teh-2`. default value is `teh-1`
+ `SMAPP_API_BASE_URL`: is the base url of smapp services. default value is `http://smapp-api.apps.inter-dc.teh-1.snappcloud.io`
...
### CallOptions
`CallOptions` is a struct that defines the behaviour of the operation. you can create a new `CallOptions` with `reverse.NewDefaultCallOptions()`
