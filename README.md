...
- [x] ETA
- [x] Matrix
- [x] SmappShot (static map image URL generation)
- [ ] Routing
...
| `WithDestinations([]Location)` | Route stops, semicolon-separated on wire (clears `Here`) |
...
Query params are sorted case-insensitively and percent-encoded. The `expires` (Unix timestamp) and `sig` params are appended automatically by `Build()`. The server validates signature and rejects expired URLs with HTTP 403.
